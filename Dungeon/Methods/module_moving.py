...
from Blocks.module_metaContainer import metaContainer
...
def moving(obj, *choice):
	...
	if not (choice[0] == 'act') and not (choice[0] == 'hit'):
		if choice[0] == 'up':
			if isinstance(obj.map[obj.row - 1][obj.elm], (Surface, Spike, Door, Stone, metaContainer)):
				obj.map[obj.row - 1][obj.elm].walk(choice[0], obj)

		elif choice[0] == 'right':
			if isinstance(obj.map[obj.row][obj.elm + 1], (Surface, Spike, Door, Stone, metaContainer)):
